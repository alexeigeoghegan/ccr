...
def get_all_market_data():
    ...
    try:
        ...
        st.sidebar.error(f"Macro Error: {e}")
    ...
    # 4. Proxies
    data_dict.update({'m2': 4.2, 'etf': 1.5, 'fund': 0.01, 'ssr': 13.5})
    return data_dict
...
if 'macro_df' in raw:
    for i in range(7):
        ...
        score = calculate_index(
            ...
            raw['fng_vals'][i] if i < len(raw['fng_vals']) else 50, raw['cbbi'], 
...
fig_gauge = go.Figure(go.Indicator(
    ...
    title = {'text': "Market Risk Level", 'font': {'size': 20}},
    gauge = {'axis': {'range': [0, 100]}, 'bar': {'color': "#00ffcc"},
             'steps': [{'range': [0, 30], 'color': '#006400'},
                       {'range': [30, 70], 'color': '#8B8000'},
                       {'range': [70, 100], 'color': '#8B0000'}]}))
fig_gauge.update_layout(paper_bgcolor='#0e1117', font={'color': "white"}, height=300, margin=dict(t=50, b=0))
...
# Safe Metric Display
m_score = (normalize(raw.get('dxy',102),98,108,True)*0.5 + normalize(raw.get('m2',4),-1,10)*0.5)
col1.metric("MACRO", f"{round(m_score, 1)}%")
col2.metric("SENTIMENT", f"{raw.get('fng_vals',[50])[0]}%")
col3.metric("TECHNICALS", f"{round(raw.get('cbbi',50), 1)}%")
col4.metric("ADOPTION", f"{round(normalize(raw.get('etf',1),-1,5), 1)}%")
col5.metric("STRUCTURE", f"{round(normalize(raw.get('fund',0.01),0,0.06,True)*0.5 + normalize(raw.get('ssr',15),8,22,True)*0.5, 1)}%")
